...
function Favoritos() {
  ...
  const [listingByGame, setListingByGame] = useState<Map<number, ListingItem[]>>(
    new Map(),
  );
  ...
  useEffect(() => {
    ...
    const carregarFavoritos = async () => {
      try {
        ...
        const [{ data: wishlistData }, { data: listingsData }, { data: cartData }] =
          await Promise.all([
  ...
  const wishlistCountLabel = useMemo(() => `${items.length} salvos`, [items.length]);
