...
export default function DetailsMid() {
  ...
  const currentListing = useMemo(() => {
    ...
    return (
      platformListings.find(
        (listing) => Number(listing.id) === selectedListingId,
      ) ?? platformListings[0]
    );
  ...
  const inCart =
    currentListingId > 0 && cartListingIds.includes(currentListingId);
  const infoItems = [
            ...
            <aside className="nexus-panel p-5 sm:p-6 lg:sticky lg:top-28 lg:h-fit">
                      ...
                      <ul className="mt-3 space-y-1 text-xs text-emerald-200">
                        {activePromotions.map((promotion) => (
                          <li key={`promo-${promotion.id}`}>
                            Promocao ativa:{" "}
                            {promotion.name || "Oferta especial"}
                          </li>
                        ))}
                      </ul>
                    )}
              ...
              <div className="mt-6 grid gap-2">
                <button
                  ...
                  disabled={
                    busyCart || inCart || availableStock <= 0 || !currentListing
                  }
                ...
                <button
                  ...
                  disabled={
                    busyBuyNow || availableStock <= 0 || !currentListing
                  }
