...
const sideCardClass = "rounded-[24px] border border-slate-800 bg-slate-900/55 p-5";
type ButtonTone = keyof typeof buttonClass;
type NoticeTone = keyof typeof noticeClass;
type FieldProps = { label: string; note?: ReactNode; className?: string; children: ReactNode };
type PageStateProps = { loading: boolean; error?: string; isEmpty?: boolean; loadingText: string; emptyText?: ReactNode; emptyClassName?: string; children: ReactNode };

export const adminFieldClass = "mt-2 w-full rounded-2xl border border-slate-700 bg-slate-900 px-4 py-3 text-white outline-none transition focus:border-blue-500/70";
export const adminFormClass = "grid gap-5 rounded-[28px] border border-slate-800 bg-slate-950/78 p-6";
...
export const AdminNotice = ({ children, tone = "error" }: { children: ReactNode; tone?: NoticeTone }) => <p className={noticeClass[tone]}>{children}</p>;
export const AdminButton = ({ tone = "primary", className, ...props }: ComponentProps<"button"> & { tone?: ButtonTone }) => <button {...props} className={cx(buttonClass[tone], className)} />;
export const AdminLinkButton = ({ tone = "secondary", className, ...props }: LinkProps & { tone?: ButtonTone }) => <Link {...props} className={cx(buttonClass[tone], className)} />;
export const AdminTextField = ({ label, note, className, ...props }: ComponentProps<"input"> & Omit<FieldProps, "children">) => <Field label={label} note={note}><input {...props} className={cx(adminFieldClass, className)} /></Field>;
export const AdminTextareaField = ({ label, note, className, ...props }: ComponentProps<"textarea"> & Omit<FieldProps, "children">) => <Field label={label} note={note}><textarea {...props} className={cx(adminFieldClass, className)} /></Field>;
...
export const AdminStatusBadge = ({ active, activeLabel = "Ativo", inactiveLabel = "Inativo" }: { active?: boolean; activeLabel?: string; inactiveLabel?: string }) => (
  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${active === false ? "border border-slate-700 bg-slate-900 text-slate-300" : "border border-blue-500/20 bg-blue-500/10 text-blue-100"}`}>
    {active === false ? inactiveLabel : activeLabel}
  </span>
);
export function AdminPageState({ loading, error, isEmpty = false, loadingText, emptyText, emptyClassName = "nexus-card p-5 text-gray-300", children }: PageStateProps) {
...
export const AdminFormActions = ({ backTo, submitLabel, saving = false, disabled = false, cancelLabel = "Cancelar", savingLabel = "Salvando..." }: { backTo: string; submitLabel: string; saving?: boolean; disabled?: boolean; cancelLabel?: string; savingLabel?: string }) => (
  <div className="flex flex-wrap gap-3">
    <AdminButton type="submit" disabled={saving || disabled}>{saving ? savingLabel : submitLabel}</AdminButton>
...
export function formatReleaseDate(value: string) {
  ...
  return year && month && day
    ? new Date(Date.UTC(year, month - 1, day)).toLocaleDateString("pt-BR", { timeZone: "UTC" })
    : "-";
}
export const getKeyStatusBadgeClass = (status: string) => `inline-flex rounded-full px-3 py-1 text-xs font-semibold ${
  status === "sold"
